...
function set_uploadfilecount() {
	var upload_txt = upload_file_count + " / " + total_intent;
	document.getElementById("upload_record").innerHTML = upload_txt;

	if (Number(upload_file_count) == Number(total_intent)) {
		enable_mTurkCode();
...
function enable_mTurkCode() {
	document.getElementById("input_survey_id").value = survey_code;
}
...
function inputCell(index, input) {
	...
	tbody.rows[index].cells[4].appendChild(input);
...
function createDownloadLink(blob) {
  ...
  filename += survey_code;
	filename += "_";

	filename += str_dom_ID;
	filename += "_";
	filename += tbody.rows[currentRadioValue].cells[1].innerHTML;
  ...
  console.log("final name : " + filename);
	...
	upload.addEventListener("click", function(event) {
		...
		fd.append("num", rows.cells[1].innerHTML);
	...
	deleteData.addEventListener("click", function(event) {
		...
		var table = document.getElementById("recordtable");
		var tbody = table.children[2];
		tbody.rows[currentRadioValue].cells[4].innerHTML = " ";
		// text number
